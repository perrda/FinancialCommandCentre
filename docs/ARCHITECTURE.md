...
> Supersedes: v1.0–v7.60 (see [`v7-architecture.md`](v7-architecture.md) for the documented v7.60 architecture)
...
|------|----------|--------|-----------|----------|
| 10 May 2026 | D1 — Build tool | Vite | Cloudflare-native, fast HMR, no real alternative | David ✓ |
| 10 May 2026 | D2 — Language | TypeScript (gradual) | Catches the bug class that's cost time historically; financial calculations need type safety | David ✓ |
| 10 May 2026 | D3 — Framework | Vue 3 | Lowest-friction migration from existing HTML/JS; mature ecosystem | David ✓ |
| 10 May 2026 | D4 — CSS strategy | CSS Modules + design tokens | Scoped, no framework lock-in, brand-friendly | David ✓ |
| 10 May 2026 | D5 — State mgmt | Pinia | Vue's standard store; minimal boilerplate | David ✓ |
| 10 May 2026 | D6 — Repo layout | pnpm monorepo | Standard for Workers+Pages; type sharing | David ✓ |
| 10 May 2026 | D7 — Testing | Vitest + Playwright | Vite-native units, real-browser E2E | David ✓ |
| 10 May 2026 | D8 — Test gate | Required | No untested modules merged | David ✓ |

---

## 10. Where we are now

D1–D8 are signed off. The `v8-foundation` branch holds the empty Phase 0 skeleton. Next: complete Phase 0 (CI green, Cloudflare Pages preview deployed), then move into Phase 1 (extract `data/` and `utils/` from `js/app.js`).

The v7.60 monolith (`index.html` + `css/style.css` + `js/app.js`) remains the live product on the `main` branch throughout the migration. Nothing changes for end users until Phase 5.
